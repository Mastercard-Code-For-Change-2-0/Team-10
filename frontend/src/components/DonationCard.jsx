    ...
    <button onClick={() => onClick?.(item)} className="card" style={{ padding: 0, textAlign: 'left' }}>
      ...
      <div style={{ padding: 14 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
          ...
          <span className="chip" style={{ fontSize: 12, color:"black"}}>{item.condition}</span>
        </div>
        <div className="muted" style={{ marginTop: 6, fontSize: 14 }}>{item.city}</div>
        <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
          <span className="chip" style={{ fontSize: 12,color:"black"}}>{item.category}</span>
          <span className="chip" style={{ fontSize: 12,color:"black" }}>Qty: {item.quantity}</span>
        </div>
      </div>
    </button>
  )
