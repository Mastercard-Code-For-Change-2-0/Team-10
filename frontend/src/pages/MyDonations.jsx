...
const statuses = ['Draft','Pending','Approved','Matched','Completed']

export default function MyDonations() {
  const [status, setStatus] = useState('All')
  const mine = useMemo(() => MOCK_DONATIONS.map(d => ({ ...d, status: statuses[(d.quantity + d.title.length) % statuses.length] })), [])
  const list = status==='All' ? mine : mine.filter(x => x.status===status)
  return (
    <main className="section"><div className="container">
      <h1 style={{ color: 'var(--brand-primary)', marginTop: 0 }}>My Donations</h1>
        ...
        {list.map(item => (
          <div key={item.id} className="card" style={{ padding: 12, display: 'grid', gridTemplateColumns: '120px 1fr auto', gap: 12, alignItems: 'center' }}>
            ...
            <div>
              ...
              <div className="muted">{item.category} • {item.condition} • Qty {item.quantity}</div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <a href={`/donations/${item.id}`} className="pill btn-outline">View</a>
              <button className="pill btn-outline">Edit</button>
              <button className="pill">Withdraw</button>
            </div>
          </div>
        ))}
      </div>
